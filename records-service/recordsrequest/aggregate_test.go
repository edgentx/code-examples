...
// released extends assigned with a release awaiting delivery.
func released() []recordsrequest.Event {
...
// compensated extends released with the compensating failure.
func compensated() []recordsrequest.Event {
	return append(released(), recordsrequest.ReleaseFailed{
		RequestID: "PRR-2026-0041",
...
// closed extends released with the delivery that closes the request.
func closed() []recordsrequest.Event {
	return append(released(), recordsrequest.DeliveryConfirmed{
		RequestID: "PRR-2026-0041",
...
func TestCommandInvariants(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "an officer command is refused while a release is out for delivery",
			history: released(),
			command: func(r *recordsrequest.Request) error {
				return r.Deny(recordsrequest.Deny{Exemption: "any", At: day3})
			},
			wantErr: recordsrequest.ErrReleaseInFlight,
		},
		{
			name:    "a delivery outcome is refused when no release is in flight",
			history: assigned(),
			command: func(r *recordsrequest.Request) error {
				return r.ConfirmDelivery(recordsrequest.ConfirmDelivery{
					PackageID: "PKG-2026-0188",
					At:        day3,
				})
			},
			wantErr: recordsrequest.ErrNotAwaitingDelivery,
		},
		{
			name:    "delivery confirmation requires a package identifier",
			history: released(),
			command: func(r *recordsrequest.Request) error {
				return r.ConfirmDelivery(recordsrequest.ConfirmDelivery{At: day3})
			},
			wantErr: recordsrequest.ErrMissingField,
		},
		{
			name:    "a compensating failure requires a reason",
			history: released(),
			command: func(r *recordsrequest.Request) error {
				return r.FailDelivery(recordsrequest.FailDelivery{At: day3})
			},
			wantErr: recordsrequest.ErrMissingField,
		},
		{
			name:    "a compensated release can be attempted again",
			history: compensated(),
			command: func(r *recordsrequest.Request) error {
				return r.Fulfill(recordsrequest.Fulfill{ReleasedPages: 16, At: day3})
			},
		},
		{
			name:    "a delivered request accepts no further commands",
			history: closed(),
			command: func(r *recordsrequest.Request) error {
				return r.Deny(recordsrequest.Deny{Exemption: "any", At: day3})
			},
		...
		{
			name:    "a delivered request accepts no second delivery",
			history: closed(),
			command: func(r *recordsrequest.Request) error {
				return r.ConfirmDelivery(recordsrequest.ConfirmDelivery{
					PackageID: "PKG-2026-0189",
					At:        day3,
				})
			},
			wantErr: recordsrequest.ErrNotAwaitingDelivery,
...
// TestCompensationWithdrawsTheRelease is the domain half of the distributed
// transaction: the compensating event does not merely annotate the request, it
// undoes the effect of the release it compensates. A page count left standing
// after an undelivered release would tell an auditor that records went out.
func TestCompensationWithdrawsTheRelease(t *testing.T) {
	request, err := recordsrequest.FromHistory(released())
	if err != nil {
		t.Fatalf("loading history: %v", err)
	}
	if request.ReleasedPages() != 18 {
		t.Fatalf("ReleasedPages before compensation = %d, want 18", request.ReleasedPages())
	}

	if err := request.FailDelivery(recordsrequest.FailDelivery{
		Reason: "two pages still under legal hold",
		At:     day3,
	}); err != nil {
	...
	if request.Status() != recordsrequest.StatusReleaseFailed {
		t.Errorf("Status = %s, want %s", request.Status(), recordsrequest.StatusReleaseFailed)
	}
	if request.ReleasedPages() != 0 {
		t.Errorf("ReleasedPages after compensation = %d, want 0", request.ReleasedPages())
	}
	if request.FailureCause() == "" {
		t.Error("FailureCause is empty; the officer is not told why the release did not land")
	}
}

// TestRetryAfterCompensationClearsTheCause proves the failure notice belongs to
// the failed attempt rather than to the request: releasing again clears it, so
// the console cannot show a stale reason beside a release that is in flight.
func TestRetryAfterCompensationClearsTheCause(t *testing.T) {
	request, err := recordsrequest.FromHistory(compensated())
	if err != nil {
		t.Fatalf("loading history: %v", err)
	}

	if err := request.Fulfill(recordsrequest.Fulfill{ReleasedPages: 16, At: day3}); err != nil {
		t.Fatalf("Fulfill: %v", err)
	}

	if request.Status() != recordsrequest.StatusReleasePending {
		t.Errorf("Status = %s, want %s", request.Status(), recordsrequest.StatusReleasePending)
	}
	if request.FailureCause() != "" {
		t.Errorf("FailureCause = %q, want empty", request.FailureCause())
	}
}
...
func TestMarkCommittedClearsPendingEvents(t *testing.T) {
	...
	if request.CommittedVersion() != request.Version() {
		t.Fatalf("CommittedVersion = %d, want %d after replay",
			request.CommittedVersion(), request.Version())
	}
	...
	// The version an append is conditioned on must not move because a command
	// was accepted in memory. Only the store moves it.
	if request.CommittedVersion() != request.Version()-1 {
		t.Errorf("CommittedVersion = %d, want %d",
			request.CommittedVersion(), request.Version()-1)
	}
	...
	if request.CommittedVersion() != request.Version() {
		t.Errorf("CommittedVersion after MarkCommitted = %d, want %d",
			request.CommittedVersion(), request.Version())
	}
	if len(pending) != 1 {
